...
export DigitalGate, DigitalCircuit, RepeatedDigitalCircuit, add_gate!, process_circuit, run_circuit_tjm, TJMOptions,
       enable_timing!, set_timing_print_each_call!, reset_timing!, print_timing_summary!

"""
Enable or disable timing collection for CircuitTJM.

This toggles timing instrumentation for CircuitTJM and propagates the setting to TDVP, dissipation,
...
"""
print_timing_summary!(; header::AbstractString="CircuitTJM timing summary", top::Int=20) =
...
"""
Configure algorithm choices for CircuitTJM gate application.
...
Returns:
    TJMOptions: Options container for CircuitTJM.
"""
struct TJMOptions
...
"""
Run CircuitTJM for a single circuit instance.
...
"""
function run_circuit_tjm(initial_state::MPS, circuit::DigitalCircuit, 
    ...
    return state, results, bond_dims
        ...
        Timing.end_scope!(ts; header="CircuitTJM per-trajectory timing")
    end
end

"""
Run CircuitTJM for a repeated circuit without materializing repeats.
...
Returns:
    Tuple: `(state, results, bond_dims)` containing the final state, observable measurements, and
        maximum bond dimensions per sample.
"""
function run_circuit_tjm(initial_state::MPS, circuit::RepeatedDigitalCircuit,
                         noise_model::Union{NoiseModel, Nothing},
                         sim_params::AbstractSimConfig;
                         alg_options::TJMOptions = TJMOptions(local_method=:TDVP, long_range_method=:TDVP))
    local ts = Timing.begin_scope!()
