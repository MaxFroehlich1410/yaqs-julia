...
def qaoa_ising_layer(n_qubits: int, beta: float | None = None, gamma: float | None = None) -> QuantumCircuit:
    """
    ...
    Args:
        ...
        beta: RX rotation angle. If None, sampled uniformly from [0, 2π).
        gamma: RZZ interaction angle. If None, sampled uniformly from [0, 2π).
...
def hea_layer(
    ...
    Args:
        n_qubits: number of qubits (≥1)
        phis: List of phi angles for Rz(phi). If None, random.
        thetas: List of theta angles for Ry(theta). If None, random.
        lams: List of lam angles for Rz(lam). If None, random.
        start_parity: 0 for even edges, 1 for odd edges. If None, random.
    ...
    for q in range(n_qubits):
        phi = phis[q] if phis is not None else rng.uniform(0.0, 2.0 * np.pi)
        theta = thetas[q] if thetas is not None else rng.uniform(0.0, np.pi)
        lam = lams[q] if lams is not None else rng.uniform(0.0, 2.0 * np.pi)
        qc.rz(phi, q)
        qc.ry(theta, q)
        qc.rz(lam, q)

    # Brickwork CZ pattern: pick parity at random for this layer
    if start_parity is not None:
        start = start_parity
    else:
