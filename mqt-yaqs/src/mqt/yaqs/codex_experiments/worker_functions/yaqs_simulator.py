...
from mqt.yaqs.core.libraries.gate_library import Z, X, Y
...
def run_yaqs(
    ...
    *,
    ...
    # Select observable based on basis
    basis_map = {"Z": Z, "X": X, "Y": Y}
    if observable_basis not in basis_map:
        raise ValueError(f"observable_basis must be one of {list(basis_map.keys())}, got {observable_basis}")
    gate_class = basis_map[observable_basis]
    obs = [Observable(gate_class(), i) for i in range(num_qubits)]
...
def build_noise_models(processes):
    ...
    # Use hazard_gain > 1 so s* = 1/gain matches theory defaults (e.g., s = 1/3)
    noise_model_unitary_2pt = NoiseModel(procs_2pt, hazard_gain=3.0)
    noise_model_unitary_gauss = NoiseModel(procs_gaus, hazard_gain=3.0, gauss_M=11)
