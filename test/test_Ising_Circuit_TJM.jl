# Scenario-style test for digital Ising TJM runs via the high-level `Simulator` API.
#
# This file executes a 6-qubit Ising circuit with a per-qubit bitflip noise model over many
# trajectories and checks that aggregated observable results are produced and remain within physical
# bounds. It is intentionally more “integration” than “unit” in nature.
#
# Args:
#     None
#
# Returns:
#     Nothing: Defines an Ising digital TJM scenario test (prints progress/output during execution).
using Test
using LinearAlgebra
using Yaqs
