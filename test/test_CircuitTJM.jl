# Unit tests for circuit TJM execution (`Yaqs.CircuitTJM`).
#
# These tests validate:
# - circuit preprocessing/layering via `process_circuit`
# - basic circuit execution (`run_circuit_tjm`) on small reference circuits (including entanglement)
# - integration with noise models in the digital trajectory-jump setting
# - the `RepeatedDigitalCircuit` wrapper (repeat without materializing copies)
#
# Args:
#     None
#
# Returns:
#     Nothing: Defines `@testset`s covering core CircuitTJM features and regressions.
using Test
using LinearAlgebra
using Yaqs
...
@testset "CircuitTJM Tests" begin
    ...
    @testset "Circuit Processing" begin
        ...
        result = CircuitTJM.process_circuit(circ)
        if result isa Tuple
            layers = result[1]
        else
            layers = result
    ...
    @testset "Run Bell State (No Noise)" begin
        ...
        psi_out, _ = run_circuit_tjm(psi, circ, nothing, sim_params)
    ...
    @testset "With Noise" begin
        ...
        # Run
        psi_out, _ = run_circuit_tjm(psi, circ, noise, sim_params)
    ...
    @testset "RepeatedDigitalCircuit" begin
        ...
        sim_params = TimeEvolutionConfig(Observable[], 1.0; dt=1.0)
        psi = MPS(L; state="zeros")
        psi_out, _ = run_circuit_tjm(psi, rep, nothing, sim_params)
