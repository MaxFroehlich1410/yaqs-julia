...
apply an MPO approximation of U = exp(-i dt H) repeatedly, compressing with the variational MPO ansatz.
...
def main() -> None:
    ...
    print(
        f"[variational] L={L} periodic Heisenberg, init state={args.state}, "
        f"E0={E0:.12g}  sweeps={args.min_sweeps}..{args.max_sweeps}"
    )
    ...
    for n in range(args.steps):
        ...
        if (n + 1) % max(1, args.steps // 5) == 0 or (n + 1) == args.steps:
            ...
            print(f"[variational] step {n+1:4d}/{args.steps}: chi_max={chi_max_now:4d}  E={E:.12g}")
    wall = time.time() - t0
    ...
    print(f"[variational] done: E={E1:.12g}  norm={norm:.12g}  chi_max={chi_max_final}  wall_s={wall:.3f}")
