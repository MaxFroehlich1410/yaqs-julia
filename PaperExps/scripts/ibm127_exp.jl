...
# Timing prints (per trajectory) from CircuitTJM
ENABLE_FUNCTION_TIMING = true
Yaqs.CircuitTJM.enable_timing!(ENABLE_FUNCTION_TIMING)
Yaqs.CircuitTJM.set_timing_print_each_call!(false)  # aggregate across trajectories
...
"""
    bond_growth_rate_per_step(bonds)

Compute a simple average growth rate per time step for a bond-dimension trace `bonds`.
We define

    (bonds[end] - bonds[begin]) / (length(bonds) - 1)

Returns `0.0` if `length(bonds) ≤ 1`.
"""
function bond_growth_rate_per_step(bonds::AbstractVector{<:Real})::Float64
    n = length(bonds)
    if n <= 1
        return 0.0
    end
    return (Float64(bonds[end]) - Float64(bonds[begin])) / (n - 1)
end
...
function perform_warmup(loc_mode, lr_mode)
    ...
    if _timing_was_enabled
        Yaqs.CircuitTJM.enable_timing!(false)
    end
    N = 4
    ...
    # Config
    opts = Yaqs.CircuitTJM.TJMOptions(local_method=Symbol(loc_mode), long_range_method=Symbol(lr_mode))
    ...
    if _timing_was_enabled
        Yaqs.CircuitTJM.enable_timing!(true)
    end
end
...
for CIRCUIT_NAME in CIRCUIT_LIST
    for NOISE_STRENGTH in NOISE_STRENGTH_LIST
        ...
        elseif BASE_MODEL == "XY"
            # Build ONE trotter step and repeat it NUM_LAYERS times in CircuitTJM.
        ...
        elseif BASE_MODEL == "IBM127_kicked_ising"
            ...
            # Build ONE kicked-Ising step and repeat it NUM_LAYERS times in CircuitTJM.
            step = DigitalCircuit(NUM_QUBITS)
            add_gate!(step, Barrier("SAMPLE_OBSERVABLES"), Int[])  # sample at t=0
            for q in 1:NUM_QUBITS
        ...
        if run_julia
            try
                ...
                if ENABLE_FUNCTION_TIMING
                    Yaqs.CircuitTJM.print_timing_summary!(header="CircuitTJM aggregated timing | Circuit=$(CIRCUIT_NAME) | Method=Julia | Noise=$(NOISE_STRENGTH)")
                end
        ...
        if run_julia_analog_2pt
            try
                ...
                if ENABLE_FUNCTION_TIMING
                    Yaqs.CircuitTJM.print_timing_summary!(header="CircuitTJM aggregated timing | Circuit=$(CIRCUIT_NAME) | Method=Julia Analog 2pt | Noise=$(NOISE_STRENGTH)")
                end
        ...
        if run_julia_analog_gauss
            try
                ...
                if ENABLE_FUNCTION_TIMING
                    Yaqs.CircuitTJM.print_timing_summary!(header="CircuitTJM aggregated timing | Circuit=$(CIRCUIT_NAME) | Method=Julia Analog Gauss | Noise=$(NOISE_STRENGTH)")
                end
        ...
        if run_julia_projector
            try
                ...
                if ENABLE_FUNCTION_TIMING
                    Yaqs.CircuitTJM.print_timing_summary!(header="CircuitTJM aggregated timing | Circuit=$(CIRCUIT_NAME) | Method=Julia Projector | Noise=$(NOISE_STRENGTH)")
                end
        ...
        # 5b. Bond-dimension growth summary (Julia unravelings only)
        # ---------------------------------------------------------
        begin
            println("\nBond dimension growth summary (Julia unravelings, averaged over trajectories):")
            rows = Vector{Tuple{String, Float64, Float64}}()  # (name, growth_rate_per_step, mean_bond)
            for (name, data) in results_detailed
                if !startswith(String(name), "Julia")
                    continue
                end
                bonds = data["bonds"]
                if isnothing(bonds)
                ...
                g = bond_growth_rate_per_step(bonds)
                μ = mean(Float64.(bonds))
                push!(rows, (String(name), g, μ))
            end

            if isempty(rows)
                println("  (No Julia bond traces were recorded.)")
            else
                sort!(rows, by = x -> x[2], rev = true) # highest growth first
                for (nm, g, μ) in rows
                    @printf "  %-20s  growth/step = %8.4f   mean χ = %8.2f\n" nm g μ
                end
                best = rows[1]
                println("Fastest bond-dimension growth: $(best[1]) (growth/step=$(best[2]))")
            end
        end
